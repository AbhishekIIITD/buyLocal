...
async function getProductOrder(request, response) {
    ...
    try {
        ...
        if (orderProducts.length === 0) {
            return response.status(404).json({ error: "Order products not found" });
        }
        ...
        // Combine order products with their respective product details
        const result = orderProducts.map(orderProduct => ({
            ...orderProduct,
        ...
        return response.status(500).json({ error: "Failed to fetch order products" });
    }
}
